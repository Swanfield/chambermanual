...
I recommend prefixing your page names with the product, such as "theme_pages" instead of just "pages." This way if you have other products that also have topics with generic names such as "pages," there won't be naming conflicts.

Additionally, consider adding the product name in parentheses after the title, such as "Pages (theme)" so that users can clearly navigate different topics for each product.
...
For details about linking to headings on different pages, see [Automated links to headings on pages][theme_hyperlinks.html#bookmarklinks].
