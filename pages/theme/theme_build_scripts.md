...
permalink: theme_build_scripts.html
folder: theme
---
...
The theme project has 5 build scripts and a script that runs them all. These scripts will require a bit of detail to configure. Every team member who is publishing on the project should set up their folder structure in the way described here.
...
For the themes project, you'll see a series of build scripts for each project. There are 5 build scripts, described in the following sections. Note that you really only need to run the last one, e.g., theme_all.sh, because it runs all of the build scripts. But you have to make sure each script is correctly configured so that they all build successfully.

{% include tip.html content="In the descriptions of the build scripts, \"theme\" is used as the sample project. Substitute in whatever your real project name is." %}

### theme_1_multiserve_pdf.sh
...
echo "Building PDF-friendly HTML site for theme Writers ..."
jekyll serve --detach --config configs/theme/config_writers.yml,configs/theme/config_writers_pdf.yml
echo "done"

echo "Building PDF-friendly HTML site for theme Designers ..."
jekyll serve --detach --config configs/theme/config_designers.yml,configs/theme/config_designers_pdf.yml
echo "done"

echo "All done serving up the PDF-friendly sites. Now let's generate the PDF files from these sites."
echo "Now run . theme_2_multibuild_pdf.sh"
```
...
Each of your configuration files needs to have a destination like this: `../doc_outputs/theme/adtruth-java`. That is, the project should build in the doc_outputs folder, in a subfolder that matches the project name.
...
### theme_2_multibuild_pdf.sh

Here's what this script looks like:

```
# Doc Writers
echo "Building the theme Writers PDF ..."
prince --javascript --input-list=../doc_outputs/theme/writers-pdf/prince-file-list.txt -o theme/files/theme_writers_pdf.pdf;
echo "done"

# Doc Designers
echo "Building theme Designers PDF ..."
prince --javascript --input-list=../doc_outputs/theme/designers-pdf/prince-file-list.txt -o theme/files/theme_designers_pdf.pdf;
echo "done"

echo "All done building the PDFs!"
echo "Now build the web outputs: . theme_3_multibuild_web.sh"
```
...
The Prince build script puts the output PDF into the theme/theme/files directory. Now you can reference the PDF file in your HTML site. For example, on the homepage you can allow people to download a PDF of the content at files/adtruth_dotnet_pdf.pdf.

### theme_3_multibuild_web.sh
...
echo "Now push the builds to the server with . theme_4_publish.sh"
```
...
scp -r -vrC ../theme_outputs/doc-writers yourusername@yourdomain:/var/www/html/yourpublishingdirectory
...
Also, in case you haven't set up the SSH publishing without a password, see [Getting around the password prompts in SCP][theme_no_password_prompts_scp]. Otherwise the script will stop and ping you to enter your password for each directory it transfers.
...
```
cd doc_outputs/theme/designers
git add --all
...
The doc_outputs/theme/designers contains the site output from theme, so when you push content from this folder into Github, you're actually pushing the HTML site output into Github, not the theme source files.
...
After you've configured all the scripts, you can run them all by running `. theme_all.sh`. You might want to run this script at lunchtime, since it may take about 10 to 20 minutes to completely build the scripts. But note that since everything is now automated, you don't have to do anything at all after executing the script. After the script finishes, everything is published and in the right location.
